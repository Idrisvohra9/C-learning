...
// By giving address to the value of the variable in the parameters of a function the value in the main function gets affected. This program shows just that.
    int x=9, y=10;
    // We will call a function to change the value of variable x by getting it's address and changing it's value
    change_value_to_2(&x);
...
//     So what it does is, we give the address at the time of calling the function and give the access to its value in parameters by placing the "*" asterisk before the parameters during prototype and declaration this is called call by reference.
// Thus, although the function is void it changes the values in the main function directly.
    return 0;
}
void swap(int *a, int *b){ //this takes the value at the predefined address that means that its not the copy but its the original value.
     *a = *a + *b;
     *b = *a - *b;
     *a = *a - *b;
